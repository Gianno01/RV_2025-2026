...
public class LoopCaller : MonoBehaviour
{
    ...
    [SerializeField] private float _secToWaitOffset;
    ...
    void OnDisable()
    {
        StopCoroutine(CallLoop());
    }

    private IEnumerator CallLoop()
    {
        yield return new WaitForSeconds(_secToWaitOffset);
        while (this.enabled && _method != null && _obj != null)
        {
            CallNow();
            yield return new WaitForSeconds(_secToWaitBeforeCall);
        } 
    }
